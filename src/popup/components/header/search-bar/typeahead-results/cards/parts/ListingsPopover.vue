...
const MAX_LISTING_COUNT = 2 // Limited space in popover

function handleClick(item: RetailerListingItem) {
  trackProductClick({
    manufacturerName: props.manufacturer,
    retailerName: item.retailer_name,
    productUrl: item.purchase_url,
    clickOrigin: `typeahead-more-listings`,
  }, activeTabId.value)
...
<template>
  <div
    class="absolute z-10 w-42 rounded-md button_primary_bg button_primary_text shadow-lg ring-1 cursor-initial
    ring-black ring-opacity-5 focus:outline-none p-2 text-sm bottom-full left-1/2 -translate-x-1/2 mb-1.5"
    role="dialog"
    aria-modal="true"
    :aria-label="$t('header.listingsPopoverTitle')"
  >
    <div class="flex justify-between items-center mb-2">
      <h3 class="font-medium whitespace-nowrap pr-3">
        {{ $t('header.listingsPopoverTitle') }}
      </h3>
      <BaseButton
        ...
        @click="emit('close')"
      >
        <BaseIcon :icon="X" class="w-4 h-4" />
      </BaseButton>
    </div>

    <ul v-if="props.listings && props.listings.length > 0" class="space-y-1.5">
      <li v-for="item in props.listings.slice(0, MAX_LISTING_COUNT)" :key="item.listing_id">
        <!-- TODO: Use BaselinkButton here - the inline-flex style on it is causing styling issues  -->
        <a
